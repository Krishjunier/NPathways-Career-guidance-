// src/pages/Psychometric/LearningStylePage.tsx
...
export default function LearningStylePage() {
    ...
    useEffect(() => {
        ...
        async function loadQuestions() {
            ...
            setError(null);
            try {
                const q = await apiService.getTestQuestions('learning');
                setQuestions(q);
                hasLoadedRef.current = true;
                const initial: AnswerMap = {};
                q.forEach((qq) => { initial[qq.id] = 0; });
                setAnswers(initial);
            } catch (err: any) {
    ...
    async function handleSubmit(e?: React.FormEvent) {
        ...
        try {
            ...
            navigate("/assessment-transition", { state: { userId, completedTestId: 'learning' } });
        } catch (err: any) {
        ...
        <div className="modern-container">
            ...
            <div className="container position-relative z-1" style={{ maxWidth: '900px' }}>
                <div className="text-center mb-5">
                    ...
                    <h1 className="fw-bold mb-2">Learning Style Assessment</h1>
                    <p className="subtitle">Discover your optimal learning strategies</p>
                </div>
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="d-flex flex-column gap-3 mb-5">
                            {questions.map((q, idx) => {
                                ...
                                return (
                                    <div key={q.id} ref={el => { questionRefs.current[idx] = el; }} className={`glass-card p-4 transition-all ${isCurrent ? 'border-primary' : ''}`} onClick={() => setCurrentQuestionIndex(idx)} style={{ opacity: isCurrent ? 1 : 0.7 }}>
